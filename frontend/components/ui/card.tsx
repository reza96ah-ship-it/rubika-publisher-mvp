...
export function SectionCard({ title, description, action, children, className = "" }: SectionCardProps) {
  return (
    <section className={`rounded-md border border-app-border bg-app-surface p-4 ${className}`}>
...
export function SurfaceCard({ children, className = "" }: { children: ReactNode; className?: string }) {
  return <div className={`rounded-md border border-app-border bg-white p-4 ${className}`}>{children}</div>;
}
