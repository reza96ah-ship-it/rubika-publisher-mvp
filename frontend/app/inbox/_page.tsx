...
export default function InboxPage() {
  ...
  const summary = [
    { label: "نیازمند اقدام", detail: "برای ادامه کار بررسی شوند", count: data.summary.action_required, icon: CircleAlert, tone: data.summary.action_required ? "text-app-danger" : "text-app-muted" },
    { label: "خوانده‌نشده", detail: "هنوز بازبینی نشده‌اند", count: unreadCount, icon: BellRing, tone: unreadCount ? "text-app-primary" : "text-app-muted" },
    { label: "هشدار worker", detail: "انتشارهای طولانی", count: data.summary.warning, icon: Clock3, tone: data.summary.warning ? "text-app-warning" : "text-app-muted" },
    { label: "اطلاع‌رسانی", detail: "نتیجه‌های موفق اخیر", count: data.summary.info, icon: CheckCircle2, tone: "text-app-success" }
  ];
        ...
        <WorkspacePage>
          ...
          <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_390px]">
            <WorkspacePanel title="جریان اعلان‌ها" description="اعلان‌ها را باز کنید تا خوانده شوند و مسیر اقدام پیشنهادی را بررسی کنید." bodyClassName="p-4">
              ...
              <div className="mt-4 overflow-hidden rounded-md border border-app-border bg-app-surface">
                  ...
                  {visibleNotifications.map((notification) => {
                    ...
                    return (
                      <button
                        ...
                        className={`app-row flex w-full items-start gap-3 px-4 py-4 text-right hover:bg-app-surfaceMuted ${active ? "bg-app-primary/10 ring-1 ring-inset ring-app-primary/20" : ""}`}
                      >
                        <span className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-md border ${notification.severity === "critical" ? "border-app-danger/20 bg-app-danger/10 text-app-danger" : notification.severity === "warning" ? "border-app-warning/20 bg-app-warning/10 text-app-warning" : "border-app-success/20 bg-app-success/10 text-app-success"}`}>
                        ...
                        <span className="min-w-0 flex-1">
                          ...
                          <span className="mt-2 block text-[11px] text-app-muted">{formatDateTime(notification.created_at)}</span>
                        </span>
                      </button>
                    );
                  })}
