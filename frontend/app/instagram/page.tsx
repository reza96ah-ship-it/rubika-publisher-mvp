...
export default function InstagramPage() {
  ...
  }, [pageId, permissions, professionalAccountId, saved, username]);
  ...
  const readiness = [
    { label: "حساب حرفه‌ای", detail: hasIdentity ? "شناسه یا نام کاربری برای مسیر OAuth ثبت شده است." : "حساب اینستاگرام Business یا Creator را مشخص کنید.", done: hasIdentity, icon: Instagram },
    { label: "مجوزهای Meta", detail: permissions.trim() ? "لیست مجوزهای موردنیاز مستند شده است." : "مجوزهای Meta Graph را مشخص کنید.", done: Boolean(permissions.trim()), icon: ShieldCheck },
    { label: "OAuth واقعی", detail: saved?.status === "connected" ? "توکن معتبر متصل است." : "در فاز بعدی باید جریان OAuth و refresh token اضافه شود.", done: saved?.status === "connected", icon: KeyRound }
  ];
  ...
  async function saveSettings(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSaved(data);
      setMessage("پروفایل اینستاگرام برای فاز اتصال واقعی آماده شد");
      showToast({ title: "تنظیمات اینستاگرام ذخیره شد", description: "زمان‌بندی واقعی بعد از Meta OAuth فعال می‌شود.", tone: "success" });
  ...
  async function testConnection() {
    ...
    try {
      ...
      showToast({ title: "اتصال اینستاگرام هنوز کامل نیست", description: "برای انتشار واقعی باید Meta OAuth و مجوزهای انتشار اضافه شود.", tone: "warning" });
      notifyWorkspaceUpdated();
    } catch (err) {
          ...
          <section className="app-studio-panel rounded-lg px-4 py-3">
            <div className="flex flex-col justify-between gap-3 lg:flex-row lg:items-center">
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-app-muted">پایه چندکاناله برای Instagram Graph API؛ انتشار واقعی بعد از OAuth و مجوزهای Meta فعال می‌شود.</p>
              </div>
          ...
          {!loading ? (
            <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_380px]">
              ...
              <aside className="space-y-4 xl:sticky xl:top-24 xl:self-start">
                <WorkspacePanel title="مسیر اتصال واقعی" description="این فاز عمداً توکن جعلی ذخیره نمی‌کند." action={<StatusToken tone="warning">OAuth pending</StatusToken>}>
                ...
                <WorkspacePanel title="قرارداد انتشار" description="قبل از اتصال واقعی، صف انتشار اینستاگرام مسدود می‌ماند.">
                  <DetailGrid
                    items={[
                      ...
                      { label: "پشتیبانی فعلی", value: "پیش‌نویس و انتخاب کانال", hint: "زمان‌بندی بعد از OAuth" },
                      { label: "مسیر بعدی", value: "Meta OAuth + publisher adapter", hint: "Graph API content publishing" }
                    ]}
                  />
                ...
                <NoticeBanner tone="warning" title="توجه">
                  انتخاب اینستاگرام در composer برای آماده‌سازی محتوا فعال است، اما زمان‌بندی آن تا اتصال واقعی Meta مسدود می‌شود.
                </NoticeBanner>
              </aside>
            </div>
          ) : null}
